...
export function BudgetInsights({ budgets, categorySpending, onClose }: BudgetInsightsProps) {
  ...
  const getAlertIcon = (type: string) => {
    switch (type) {
      case 'danger': return <AlertTriangle className="h-5 w-5 text-foreground" />;
      case 'warning': return <AlertTriangle className="h-5 w-5 text-foreground" />;
      case 'success': return <CheckCircle className="h-5 w-5 text-foreground" />;
      case 'info': return <Bell className="h-5 w-5 text-foreground" />;
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      case 'increasing': return <TrendingUp className="h-4 w-4 text-foreground" />;
      case 'decreasing': return <TrendingDown className="h-4 w-4 text-foreground" />;
              ...
              {alerts.map((alert) => (
                ...
                >
                  ...
                  <div className={`absolute top-0 right-0 w-20 h-20 rounded-full opacity-10 ${
                    alert.type === 'danger' ? 'bg-muted' :
                    alert.type === 'warning' ? 'bg-muted' :
                    alert.type === 'success' ? 'bg-muted' : 'bg-muted'
                  ...
                  <div className="relative flex items-start justify-between">
                    <div className="flex items-start gap-3 flex-1">
                      <div className={`h-10 w-10 rounded-lg flex items-center justify-center shadow-sm ${
                        alert.type === 'danger' ? 'bg-muted text-foreground' :
                        alert.type === 'warning' ? 'bg-muted text-foreground' :
                        alert.type === 'success' ? 'bg-muted text-foreground' : 'bg-muted text-foreground'
                      }`}>
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${
                      prediction.trend === 'increasing' ? 'bg-muted text-foreground' :
                      prediction.trend === 'decreasing' ? 'bg-muted text-foreground' :
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-medium text-foreground">{Math.round(prediction.confidence)}%</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="relative">
              <div className="text-2xl font-bold text-foreground mb-1">{alerts.filter(a => a.type === 'danger').length}</div>
          ...
          >
            <div className="absolute top-0 right-0 w-12 h-12 bg-muted rounded-full -mr-6 -mt-6 group-hover:scale-110 transition-transform"></div>
            <div className="relative">
              <div className="text-2xl font-bold text-foreground mb-1">{alerts.filter(a => a.type === 'warning').length}</div>
          ...
          >
            <div className="absolute top-0 right-0 w-12 h-12 bg-muted rounded-full -mr-6 -mt-6 group-hover:scale-110 transition-transform"></div>
            <div className="relative">
              <div className="text-2xl font-bold text-foreground mb-1">{predictions.filter(p => p.trend === 'decreasing').length}</div>
